...
type Logger struct {
	...
	ReportCaller bool
	// The minimum level to report caller
	FilelineNumberLevel Level
...
func New() *Logger {
	return &Logger{
		...
		ExitFunc:            os.Exit,
		ReportCaller:        false,
		FilelineNumberLevel: ErrorLevel,
	}
}
...
func (logger *Logger) SetOutput(output io.Writer) {
	logger.mu.Lock()
	defer logger.mu.Unlock()
	logger.Outs = []io.Writer{output}
}

// SetOutputs sets the logger multiple outputs
...
func (logger *Logger) SetFilelineNumberLevel(level Level) {
	logger.mu.Lock()
	defer logger.mu.Unlock()
	logger.FilelineNumberLevel = level
}
